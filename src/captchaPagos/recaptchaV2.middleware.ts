...
const RECAPTCHA_SECRET_KEY = (
  process.env.RECAPTCHA_SECRET_KEY_MINE ||
...
export const verifyRecaptchaV2 = async (req: Request, res: Response, next: NextFunction) => {
  ...
  if (!recaptchaToken) {
    return res
      ...
      .json({
        message:
          'Error de seguridad: Falta el token CAPTCHA. Por favor, marca la casilla "No soy un robot".',
      });
  }
  ...
  try {
    ...
    const response = await axios.post<RecaptchaResponse>(verificationURL, params, {
    ...
    if (!data.success) {
      console.warn(
        `Bloqueando solicitud: Verificación CAPTCHA fallida. Clave usada: ...${RECAPTCHA_SECRET_KEY.slice(-5)}`,
        data['error-codes'],
      );
