...
export async function verifyRecoveryCodeForEmail(email: string, codigo: string) {
  try {
    ...
    const hashIngresado = crypto.createHash('sha256').update(codigo).digest('hex');

    // Buscar coincidencia
    const index = user.recoveryCodes.findIndex((storedHash) => storedHash === hashIngresado);
    ...
    await db
      .collection('users')
      .updateOne({ _id: new ObjectId(user._id) }, { $set: { recoveryCodes: user.recoveryCodes } });
