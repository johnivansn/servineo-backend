...
interface InvoiceDocument extends Document {
  ...
  items: IDetailItem[];
  notes: string; // Campos para compatibilidad con el frontend si fuera necesario
  taxRate?: number;
  taxAmount?: number;
...
export const getInvoices = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (!authenticatedRequesterId) {
      return res
        .status(401)
        .json({ success: false, message: 'Requester ID es obligatorio para cargar facturas.' });
    }
...
export const getInvoiceDetail = async (req: Request, res: Response, next: NextFunction) => {
  ...
  if (!authenticatedRequesterId) {
    return res
      .status(401)
      .json({ success: false, message: 'Requester ID es obligatorio para acceder al detalle.' });
  }
  ...
  try {
    ...
    if (!invoice) {
      return res
        .status(404)
        .json({ success: false, message: 'Factura no encontrada o acceso denegado.' });
    }
    // ====================================================
    ...
    if (!invoice) {
      return res
        .status(500)
        .json({ success: false, message: 'Error interno al marcar la factura como descargada.' });
    ...
    const taxRate = 0.16;
    const taxAmount =
      invoice.total - calculatedSubtotal > 0 ? invoice.total - calculatedSubtotal : 0;
